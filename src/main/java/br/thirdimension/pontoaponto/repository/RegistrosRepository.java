...
public interface RegistrosRepository extends PagingAndSortingRepository<Registros, Long>{
    ...
    Registros buscarRegistroPorData(LocalDate data, Usuario usuario);
    ...
    @Query("SELECT r FROM Registros r WHERE r.usuario = ?1 and MOD((select count(d) from RegistrosDia d where d.registros = r),2) = 0 order by r.dataRegistro asc")
